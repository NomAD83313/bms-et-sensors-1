...
NODE_DIR="${PROJECT_ROOT}/nodes/esp32c3SuperMini/matter-node"
...
start_epoch="$(date +%s)"
echo ">>> Building ESP32-C3-SuperMini firmware (timeout=${BUILD_TIMEOUT_SEC}s)..."
...
printf '>>> Firmware build ready in %02d:%02d:%02d\n' \
  ...
  "$(((elapsed_sec % 3600) / 60))" \
  "$((elapsed_sec % 60))"
echo ">>> App image: ${NODE_DIR}/build/esp32c3_matter_node.bin"

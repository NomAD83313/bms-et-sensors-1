...
NODE_DIR="${PROJECT_ROOT}/nodes/esp32c6zero/multinode"
...
start_epoch="$(date +%s)"
echo ">>> Building ESP32-C6-Zero Multinode firmware (timeout=${BUILD_TIMEOUT_SEC}s)..."
...
printf '>>> Firmware build ready in %02d:%02d:%02d\n' \
  ...
  "$(((elapsed_sec % 3600) / 60))" \
  "$((elapsed_sec % 60))"
echo ">>> App image: ${NODE_DIR}/build/esp32c6_multinode.bin"

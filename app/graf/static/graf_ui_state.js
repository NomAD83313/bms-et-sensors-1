...
function normalizeTempChannelSelection(raw) {
  ...
  for (let i = 0; i < 8; i += 1) {
    const key = `ch${i}`;
    out[key] = !raw || raw[key] !== false;
  }
  if (raw && typeof raw === "object") {
    Object.entries(raw).forEach(([key, value]) => {
      if (String(key).includes("|")) out[key] = value !== false;
    });
  }
  return out;
}
...
async function loadTempChannelSelection() {
  try {
    ...
    tempChannelSelection = normalizeTempChannelSelection((data && data.channels) || {});
  } catch (_) {
...
async function saveTempChannelSelection() {
  try {
    await fetch("api/redlab/channels", {
      ...
      body: JSON.stringify(tempChannelSelection),
    });
  } catch (_) {}
}
